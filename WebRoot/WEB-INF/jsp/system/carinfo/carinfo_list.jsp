...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="carinfo/listUsers.do" method="post" name="userForm" id="userForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="display:none;">车辆ID</th>
									<th class="center">车牌号</th>
									<th class="center">行驶证号</th>
									<th class="center">行驶证照片</th>
									<th class="center">保险日期</th>
									<th class="center">司机ID</th>
									<th class="center">车主ID</th>
									<th class="center">车辆状态</th>
									<th class="center">车辆类型</th>
									<th class="center">车辆载重</th>
								</tr>
							</thead>
							<tbody>
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty carinfoList}">
									<%-- <c:if test="${QX.cha == 1 }"> --%>
									<c:forEach items="${carinfoList}" var="carinfo" varStatus="vs">
										<tr>
											...
											<td class="center" style="display:none;">${carinfo.CARID }</td>
											<td class="center">${carinfo.CARNUM }</td>
											<td class="center">${carinfo.LICENSEID }</td>
											<td class="center">${carinfo.LICENSEPHOTOPATH }</td>
											<td class="center">${carinfo.INSURANCEDATE }</td>
											<td class="center">${carinfo.DRIVERID }</td>
											<td class="center">${carinfo.OWNERID }</td>
											<td class="center">${carinfo.STATUS }</td>
											<td class="center">${carinfo.CARTYPE }</td>
											<td class="center">${carinfo.CARLOAD }</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="update('${carinfo.CARID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<%-- </c:if>
													<c:if test="${QX.del == 1 }"> --%>
													<a class="btn btn-xs btn-danger" onclick="deleteU('${carinfo.CARID}','${carinfo.CARID }');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.edit == 1 }"> --%>
															<li>
																<a style="cursor:pointer;" onclick="update('${carinfo.CARID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }"> --%>
															<li>
																<a style="cursor:pointer;" onclick="deleteU('${carinfo.CARID }','${carinfo.CARID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<%-- <c:if test="${QX.add == 1 }"> --%>
									<a class="btn btn-mini btn-success" onclick="carinfo_save();">新增</a>
									<%-- </c:if>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		//新增
		function carinfo_save(){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增资源";
			 diag.URL = '<%=basePath%>carinfo/carinfosave.do';
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function update(CARID){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改资料";
			 diag.URL = '<%=basePath%>carinfo/update.do?CARID='+CARID;
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
					 dnowPage=${page.currentPage};
				}
				diag.close();
			 };
			 diag.show();
		...
		//删除
		function deleteU (CARID,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>carinfo/delete.do?CARID="+CARID;
