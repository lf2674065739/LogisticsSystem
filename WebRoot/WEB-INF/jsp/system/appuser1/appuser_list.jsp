...
<body class="no-skin">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="display:none;">用户ID</th>
									<th class="center">姓名</th>
									<th class="center">角色</th>
									<th class="center">电话</th>
									<!-- <th class="center">所属区域</th>
									<th class="center">上级机构</th> -->
							...
							<c:choose>
								<c:when test="${not empty appUserList}">
									<%-- <c:if test="${QX.cha == 1 }"> --%>
									<c:forEach items="${appUserList}" var="appUser" varStatus="vs">
										<tr>
											...
											<td class="center" style="display:none;">${appUser.USERID }</td>
											<td class="center">${appUser.NAME }</td>
											<td class="center">
											    <c:if test="${appUser.ROLEID == '1' }">
												     <span>司机</span>
												</c:if>
												<c:if test="${appUser.ROLEID == '2' }">
												     <span>车主</span>
												</c:if>
												<c:if test="${appUser.ROLEID == '3' }">
												     <span>发货端</span>
												</c:if>
												<c:if test="${appUser.ROLEID == '4' }">
												     <span>收货端</span>
												</c:if>
												<c:if test="${appUser.ROLEID == '5' }">
												     <span>物流</span>
												</c:if>
											</td>
											<td class="center">${appUser.PHONE }</td>
											<%-- <td class="center">${appUser.AREAID }</td>
											<td class="center">${appUser.ORGID }</td> --%>
											<td class="center">${appUser.LASTTIME }</td>
											<td class="center">
											    <c:if test="${appUser.STATUS == '0' }">
												     <span class="label label-success arrowed">正常</span>
												</c:if>
												<c:if test="${appUser.STATUS == '1' }">
												     <span class="label label-important arrowed-in">失效</span>
												</c:if>
											</td>
											<td class="center">
												<c:if test="${appUser.CHECKIN == '2' }">
												     <span class="label label-important arrowed-in">未审核</span>
												</c:if>
												<c:if test="${appUser.CHECKIN == '3' }">
												     <span class="label label-info arrowed">审核未通过</span>
												</c:if>
												<c:if test="${appUser.CHECKIN == '4' }">
												     <span class="label label-success arrowed">审核通过</span>
												</c:if>
											</td>
											<%-- <td class="center">${appUser.CHECKIN}</td> --%>
											<td class="center">
												<%-- <c:if test="${QX.edit != 1 && QX.del != 1 }">
												<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
												</c:if> --%>
												<div class="hidden-sm hidden-xs btn-group">
												    <a class="btn btn-xs btn-info" title="审核" onclick="checkAppUser('${appUser.USERID}');">
													...
													<%-- </c:if>
													<c:if test="${QX.del == 1 }"> --%>
													<a class="btn btn-xs btn-danger" onclick="delAppUser('${appUser.USERID }','${appUser.NAME }');">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<!-- <a class="btn btn-mini btn-success" onclick="uploadCardId('上传身份证照片');">上传身份证照片</a>
									<a class="btn btn-mini btn-success" onclick="add();">上传驾驶证照片</a>
									<a class="btn btn-mini btn-success" onclick="add();">上传法人身份证照片</a>
									<a class="btn btn-mini btn-success" onclick="add();">上传法营业执照照片</a> -->
									<%-- </c:if> --%>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		//去审核界面
		function checkAppUser(USERID){
			top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="审核界面";
			 diag.URL = '<%=basePath%>appUser/goCheckAppUser.do?USERID='+USERID;
			 diag.Width = 600;
			 diag.Height = 800;
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增会员";
			 diag.URL = '<%=basePath%>appUser/goAddAppUser.do';
			 diag.Width = 600;
			 diag.Height = 350;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//删除
		function delAppUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>appUser/deleteAppUser.do?USERID="+userId+"&tm="+new Date().getTime();
		...
		//去上传身份证照片页面
		function uploadCardId(title){
			//alert(document.getElementsByName('ids')[0].PHONE);
			top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title =title;
			 diag.URL = '<%=basePath%>appUser/goUploadCardId.do?PHONE=21';
			 diag.Width = 800;
			 diag.Height = 490;
			 diag.CancelEvent = function(){ //关闭事件
				 if('${page.currentPage}' == '0'){
