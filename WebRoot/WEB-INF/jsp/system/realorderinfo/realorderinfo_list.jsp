...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="realorderinfo/listUsers.do" method="post" name="userForm" id="userForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="display:none;">订单ID</th>
									<th class="center">订单号</th>
									<th class="center">车辆ID</th>
									<th class="center">司机ID</th>
									<th class="center">订单货物</th>
									<th class="center">货主方货物重量</th>
									<th class="center">收货方货物重量</th>
									<th class="center">订单生成时间</th>
									<th class="center">订单失效时间</th>
									<th class="center">订单状态</th>
									<th class="center">物流ID</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty realorderinfoList}">
									<%-- <c:if test="${QX.cha == 1 }"> --%>
									<c:forEach items="${realorderinfoList}" var="realorderinfo" varStatus="vs">
										<tr>
											...
											<td class="center" style="display:none;">${realorderinfo.ORDERID}</td>
											<td class="center">${realorderinfo.ORDERNUM }</td>
											<td class="center">${realorderinfo.CARID }</td>
											<td class="center">${realorderinfo.DRIVERID }</td>
											<td class="center">${realorderinfo.ORDERGOODS }</td>
											<td class="center">${realorderinfo.SHIPPERGOODSWEIGHT }</td>
											<td class="center">${realorderinfo.RECEIPTGOODSWEIGHT }</td>
											<td class="center">${realorderinfo.ORDERCREATEDATE }</td>
											<td class="center">${realorderinfo.ORDERFAILUREDATE }</td>
											<td class="center">${realorderinfo.ORDERSTATUS }</td>
											<td class="center">${realorderinfo.LOGISTICSID }</td>
											<td class="center">${realorderinfo.FREIGHT }</td>
											<td class="center">${realorderinfo.FINISHTIME }</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="update('${realorderinfo.ORDERID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<%-- </c:if>
													<c:if test="${QX.del == 1 }"> --%>
													<a class="btn btn-xs btn-danger" onclick="deleteU('${realorderinfo.ORDERID}','${realorderinfo. ORDERID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.edit == 1 }"> --%>
															<li>
																<a style="cursor:pointer;" onclick="update('${realorderinfo.ORDERID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }"> --%>
															<li>
																<a style="cursor:pointer;" onclick="deleteU('${realorderinfo.ORDERID }','${realorderinfo.ORDERID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<%-- <c:if test="${QX.add == 1 }"> --%>
									<a class="btn btn-mini btn-success" onclick="realorderinfo_save();">新增</a>
									<%-- </c:if>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		//新增
		function realorderinfo_save(){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增资源";
			 diag.URL = '<%=basePath%>realorderinfo/realorderinfosave.do';
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function update(ORDERID){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改资料";
			 diag.URL = '<%=basePath%>realorderinfo/updateU.do?ORDERID='+ORDERID;
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
		...
		//删除
		function deleteU (ORDERID,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>realorderinfo/delete.do?ORDERID="+ORDERID;
