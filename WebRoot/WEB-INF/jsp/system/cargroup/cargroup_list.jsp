...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="cargroup/listUser.do" method="post" name="userForm" id="userForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center" style="display:none;">组ID</th>
									<th class="center">组名称</th>
									<th class="center">组描述</th>
									<th class="center">组排序</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty cargroupList}">
									<%-- <c:if test="${QX.cha == 1 }"> --%>
									<c:forEach items="${cargroupList}" var="cargroup" varStatus="vs">
										<tr>
											...
											<td class="center" style="display:none;">${cargroup.GROUPID}</td>
											<td class="center">${cargroup.GROUPNAME}</td>
											<td class="center"> ${cargroup.GROUPDES}</td>
											<td class="center">${cargroup.GROUPSORT}</td>
											  <td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="update('${cargroup.GROUPID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<%-- </c:if>
													<c:if test="${QX.del == 1 }"> --%>
													<a class="btn btn-xs btn-danger" onclick="deleteU('${cargroup.GROUPID}','${cargroup.GROUPID }');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.edit == 1 }"> --%>
															<li>
																<a style="cursor:pointer;" onclick="update('${cargroup.GROUPID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }"> --%>
															<li>
																<a style="cursor:pointer;" onclick="deleteU('${cargroup.GROUPID }','${cargroup.GROUPID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<%-- <c:if test="${QX.add == 1 }"> --%>
									<a class="btn btn-mini btn-success" onclick="cargroup_save();">新增</a>
									<%-- </c:if>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		//新增
		function cargroup_save(){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增资源";
			 diag.URL = '<%=basePath%>cargroup/carsave.do';
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function update(GROUPID){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改资料";
			 diag.URL = '<%=basePath%>cargroup/goEdit.do?GROUPID='+GROUPID;
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
		...
		//删除
		function deleteU(GROUPID,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>cargroup/deleteU.do?GROUPID="+GROUPID;
